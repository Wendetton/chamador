    ...
    <div className="container mx-auto p-4 bg-gray-100 min-h-screen">
          ...
          <div key={c.id} className="bg-yellow-100 p-4 rounded flex justify-between items-center animate-pulse">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                {formatTime(c.chamadoEm)} - Criado por {c.createdBy}
              </p>
            </div>
            <button
      ...
      <div className="bg-white p-4 rounded mb-6">
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-xl font-semibold text-black">Histórico de Chamadas</h2>
          <button
          ...
          <ul className="space-y-2 max-h-64 overflow-auto">
            {[...documentData.history].reverse().map(c => (
              <li key={c.id} className="border p-2 rounded text-black">
                {c.nome} - Sala {c.sala} {c.observacao && `(Obs: ${c.observacao})`} - 
