...
import { doc, onSnapshot, setDoc, updateDoc, collection, deleteDoc, serverTimestamp, query, where, getDocs, arrayUnion } from 'firebase/firestore';
...
export default function CallPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeDoc = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        const currentData = {
          currentCall: data.currentCall || { nome: '', sala: '', chamadoEm: null },
          history: data.history || [],
        };
        setCallData(currentData);
      } else {
        setDoc(docRef, initialCallData).then(() => {
  ...
  return (
    <div className="container mx-auto p-4 flex flex-col min-h-screen bg-gray-100">
      ...
      {callData.currentCall && callData.currentCall.nome && (
        <div className="bg-green-100 p-6 rounded shadow-md mb-6 animate-pulse">
          <h2 className="text-2xl font-bold text-green-700">Chamando: {callData.currentCall.nome}</h2>
          <p className="text-lg text-green-600">Sala: {callData.currentCall.sala}</p>
          <p className="text-sm text-gray-600">{calcularTempo(callData.currentCall.chamadoEm)}</p>
        </div>
      )}
      ...
      <div className="bg-white p-6 rounded shadow-md">
        ...
        {callData.history.length > 0 ? (
          <ul className="space-y-2 max-h-80 overflow-y-auto">
            {[...callData.history].reverse().map((chamada, index) => (
