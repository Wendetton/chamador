...
export default function CallPage() {
  ...
  const presenceIntervalRef = useRef();
  ...
  useEffect(() => {
    ...
    if (!localUserId) {
      localUserId = Math.random().toString(36).slice(2);
      localStorage.setItem('collab_user_id', localUserId);
    }
    setUserId(localUserId);
    if (!localUserName) {
      localUserName = prompt("Digite seu nome:") || `Anônimo-${localUserId.slice(0,4)}`;
  ...
  useEffect(() => {
    ...
    const unsubDoc = onSnapshot(docRef, snap => {
      if (snap.exists()) setDocumentData(snap.data());
      else setDoc(docRef, initialDocumentData);
      setLoading(false);
    });
    ...
    const unsubUsers = onSnapshot(usersRef, snap => {
      ...
      snap.forEach(doc=> {
        const d = doc.data();
        if (d.lastSeen?.toDate() > cutoff) users.push({id: doc.id, ...d});
      });
    ...
    return () => {
      unsubDoc(); unsubUsers(); clearInterval(presenceIntervalRef.current);
  ...
  const handleCall = () => {
    ...
    updateField('history',[...(documentData.history||[]),call]);
    setNome(''); setSala('');
  };

  const colors = { red: {bg:'bg-red-100',text:'text-red-700'}, yellow:{bg:'bg-yellow-100',text:'text-yellow-700'}, green:{bg:'bg-green-100',text:'text-green-700'} };
  const { bg, text } = colors[documentData.currentCall.color]||colors.green;
  const tempo = t=> {
    const d = Math.floor((Date.now()-new Date(t))/60000);
    return d?'Há '+d+' min':'Agora';
  };
    ...
    <div className="container mx-auto p-4 bg-gray-100 min-h-screen">
      ...
      <div className="bg-white p-6 rounded shadow mb-6">
        ...
        <div className="flex gap-4 flex-col md:flex-row">
          <input type="text" placeholder="Nome do paciente" value={nome} onChange={e=>setNome(e.target.value)} className="border px-4 py-2 rounded text-black placeholder-black flex-1"/>
          <input type="text" placeholder="Sala" value={sala} onChange={e=>setSala(e.target.value)} className="border px-4 py-2 rounded text-black placeholder-black w-32"/>
          <select value={selectedColor} onChange={e=>setSelectedColor(e.target.value)} className="border px-2 py-2 rounded text-black">
            <option value="red">Vermelho</option>
            <option value="yellow">Amarelo</option>
            <option value="green">Verde</option>
          </select>
          <button onClick={handleCall} className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Chamar</button>
        </div>
      </div>
      <div className={`${bg} p-6 rounded shadow mb-6 animate-pulse`}>
        ...
        <p className="text-sm text-gray-600">{tempo(documentData.currentCall.chamadoEm)}</p>
      </div>
          ...
          <ul className="space-y-2 max-h-80 overflow-auto">
            {[...documentData.history].reverse().map((ch,i)=>(
              <li key={i} className="border p-3 rounded">
                ...
                <p className="text-black text-xs">{tempo(ch.chamadoEm)}</p>
              </li>
            ))}
          </ul>
        ...
        <ul className="space-y-2 max-h-60 overflow-auto">
          {activeUsers.map(u=>(
            <li key={u.id} className="flex items-center p-2 rounded hover:bg-gray-100">
              <span className="inline-block w-2 h-2 rounded-full mr-2" style={{backgroundColor:`${u.color}1A`}}/>
