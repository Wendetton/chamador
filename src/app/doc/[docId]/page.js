...
export default function CallPage() {
  ...
  useEffect(() => {
    ...
    let localColor = localStorage.getItem('collab_user_color');
    if (!localId) {
    ...
    setUserId(localId);
    if (!localName) {
      localName = prompt("Seu nome:") || `Anônimo-${localId.slice(0,4)}`;
      localStorage.setItem('collab_user_name', localName);
    }
    setUserName(localName);
    if (!localColor) {
      localColor = getRandomColor();
      localStorage.setItem('collab_user_color', localColor);
    }
      ...
      <div className="bg-white p-6 rounded shadow mb-6">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <input
            ...
            className="border px-4 py-2 rounded text-black placeholder-black flex-1"
          />
          <input
            type="text"
            placeholder="Sala"
            ...
            className="border px-4 py-2 rounded text-black placeholder-black w-32"
          />
          <select
            value={selectedColor}
